...
class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        for num in nums:
            num_count[num] += 1
            heapq.heappush(pq, (-num_count[num], num))

        ans_set = set()
        while len(pq) > 0 and len(ans_set) < k:
            ans_set.add(heapq.heappop(pq)[1])
    ...
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        def partition(left, right, pivot_index) -> int:
            ...
            for index in range(left, right):
                if count[unique[index]] < pivot_frequency:
